  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const processedNewCourses = allNewCourses.map(course => ({
          ...course,
          color: course.color || getRandomColor(),
          badge: course.badge || 'New',
          priceText: course.price ? `Rs. ${course.price.toLocaleString()}` : 'Free'
        }));

        const processedEnrolled = enrolled.map(course => ({
          ...course,
          color: course.color || getRandomColor()
        }));
